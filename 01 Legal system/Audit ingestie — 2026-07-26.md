...
You uploaded raw legis.md dumps into repo-root `99 Attachments/source-legis/` (filenames = `doc_id.md`).

The ingestion pipeline:

1. Kept each raw dump (now consolidated under [[SOURCE INDEX|`99 Attachments/source-legis/`]]).
2. Converted a working copy into `10 Legislation/… (text).md` with frontmatter + `### Articolul N.` anchors.
...
| Raw dumps lived at repo-root `99 Attachments/source-legis/` while working texts lived under `Moldovan Law/10…` | Structure | Moved all originals to `Moldovan Law/99 Attachments/source-legis/` + [[SOURCE INDEX]] |
...
```
MD-Law/                                ← open THIS as the Obsidian vault
├── Home.md · Conventions.md
├── 00 Inbox/                          ← Roadmap, capture, unfiled
├── 01 Legal system/                   ← meta, status, this audit
├── 10 Legislation/                    ← WORKING texts + notes
│   ├── Constitution/
│   ├── Codes/                         ← Civil (by Carte), Fiscal (by Titlu), etc.
│   ├── Laws/
│   ├── Government Decisions/
│   └── Authority Acts/
├── 20 Domains/ · 20 Position Papers/
├── 30 Concepts/ · 50 MOCs/ · 60 Authorities/
├── 90 Templates/
├── 99 Attachments/source-legis/       ← ALL original legis.md dumps
│   └── SOURCE INDEX.md
└── uploads/                           ← drop zone (cleared after each batch)
```

Same skeleton as the EU / Romanian vaults — jurisdiction-specific labels only under `10 Legislation`.
...
## See also
[[SOURCE INDEX]] · [[Status ingestie — Energetica]] · [[Conventions]] · [[Roadmap]]

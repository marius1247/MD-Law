---
title: "HG 854-2006 — transporturi auto calatori (notă)"
type: act-note
act: "[[HG 854-2006 — transporturi auto calatori (text)]]"
domeniu: ['transport', 'rutier']
enabling_act: "[[Legea 1194-1997 — transporturi (text)]]"
analysis_tier: law-house
tags: [act-note, analysis, uploads-pending]
status: draft
upload_source: "Uploads/HG 854-2006.md"
created: 2026-07-28
updated: 2026-07-28
---

# Passenger Road Transport Regulation — Analysis

**Raw text:** [[HG 854-2006 — transporturi auto calatori (text)]] ⚠️ *pending ingestion from Uploads* · **Hub:** [[MOC — Drept comercial]]

> [!warning] Text not yet ingested
> Raw legis.md dump in `Uploads/`. Working `(text)` file to be created per [[Status ingestie — Uploads]].

> [!abstract] Executive summary & commercial impact
> **Core purpose:** Government regulation on scheduled and occasional passenger road transport: licensing, route authorisation, tariffs, and passenger rights.
> **Primary business risk:** See risk matrix below.

---

## 1. Statutory hierarchy & legal foundation

* **Enabling / related framework:** [[Legea 1194-1997 — transporturi (text)]]
* **Procedural challenges:** [[Codul administrativ 116-2018 (notă)]]; [[Codul de procedura civila 225-2003 (notă)]].
* **EU / international alignment:** EU Regulation 181/2011 passenger rights

---

## 2. Practical legal mechanics & key provisions

*Detailed article-level analysis pending full text ingestion.*

---

## 3. Legal ambiguities, vulnerabilities & risk matrix

| Provision | Flaw / ambiguity | Practical risk | Mitigation |
| :--- | :--- | :--- | :--- |
| **Route licence** | Operating without route authorisation | Vehicle seizure | Obtain ARTA/autority route permit before operations |
| **Tariff** | Unapproved fare structures | Administrative fines | Submit tariff schedules for approval |
| **Passenger rights** | Denial of boarding and refund disputes | Compensation claims | Publish passenger rights notice in vehicles |

---

## 4. Relevant jurisprudence & practice

*To be enriched after text ingestion and case research.*

---

## 5. Client action checklist / compliance roadmap

- [ ] Obtain passenger transport licence
- [ ] Register routes with competent authority
- [ ] Display approved tariffs
- [ ] Maintain vehicle and driver qualification files

---

## 6. Connections in the vault

* **Hub:** [[MOC — Drept comercial]]
* **Ingestion:** [[Status ingestie — Uploads]]

---

## Sources

`Uploads/HG 854-2006.md` — pending migration to `10 Acte normative/`.

---
title: "Concept — Situații financiare"
type: concept
domeniu: [contabilitate, societăți]
tags: [concept, contabilitate, societăți]
status: draft
created: 2026-07-23
---

# Concept — Situații financiare

**RO:** situații financiare · **EN:** financial statements

## Definition
The annual set of statements an entity must prepare under **Legea contabilității și raportării financiare nr. 287/2017**: balance sheet, profit and loss account, and — depending on category — statement of changes in equity, cash flow statement, and notes.

## Why they are a legal document, not an accounting one
The statements are the **factual input** to a series of legal determinations:

| They determine | Consequence |
|---|---|
| **Distributable profit** | What may lawfully be paid to owners → [[Concept — Adunarea generală]] |
| **Net assets vs share capital** | Triggers a mandatory decision to recapitalise, reduce or dissolve → [[Concept — Capital social]] |
| **Taxable profit (as the starting point)** | Adjusted under the Fiscal Code → [[Concept — Impozit pe venit]] |
| **Insolvency in fact** | The forensic date that sets the suspect period → [[Concept — Insolvabilitate]] |
| **Director compliance** | Failure to keep proper accounts is itself a breach → [[Concept — Administrator]] |

## Reporting framework
- **SNC** — national accounting standards, the default
- **IFRS** — mandatory for **public interest entities** (banks, insurers, others designated by law); available to others by election

Medium and large entities prepare **complete statements under SNC**; micro and small entities may use abridged forms.

## Size categories drive the obligation
Category is determined by not exceeding **two of three** criteria — total assets, sales revenue, average employees.

> [!important] Thresholds change on **1 January 2027**
> **Legea nr. 86 of 21 May 2026** raises them substantially: **micro** ≤ MDL 8.5M assets / 17M revenue; **small** ≤ 95M / 190M; **medium** ≤ 480M / 960M. Many entities will move **down** a category, reducing reporting and audit obligations in one step. Verify the headcount criterion and the transitional rules at source. → [[Contabilitate & raportare financiară — sinteza]]

**Groups:** small and medium groups are **exempt from consolidated statements** — unless a member is a public interest entity, which pulls the whole group into consolidation.

## The annual cycle
1. **Close the accounts** and prepare the statements
2. **Audit**, where required by category or sector
3. **Approve** at the general meeting — approval is a reserved competence, and the profit-distribution resolution rides on it
4. **File** with the state financial reporting system
5. **Publish**, where required

## Practical points
- **Responsibility rests with management**, not with the accountant or the outsourced provider. Delegation does not transfer it.
- **Keep the tax reconciliation** — accounting profit to taxable profit. It is the first document [[SFS]] requests in a control.
- **Foreign currency** translation and exchange differences are a material P&L item in a volatile-currency environment, not a footnote.
- In **due diligence**, filed statements are the counterparty's own admissions about its position — read them against the [[ASP]] register extract.

## Governing provisions *(to ingest)*
- **Legea contabilității și raportării financiare nr. 287/2017**, and amending **Legea nr. 86/2026**
- **SNC** — approved by Ministry of Finance order, a layer below the law
- **Codul fiscal** — the adjustments from accounting to taxable profit
- **Legea nr. 1134/1997** / **Legea nr. 135/2007** — approval and distribution

## Related
[[Concept — Capital social]] · [[Concept — Adunarea generală]] · [[Concept — Administrator]] · [[Concept — Insolvabilitate]] · [[Concept — Impozit pe venit]] · [[Contabilitate & raportare financiară — sinteza]] · [[SFS]]

## Notes / conclusions
> Financial statements are where company law, tax law and insolvency law all read from the same page. That is exactly why the accounting regime deserves legal attention rather than being delegated to the accountants.
